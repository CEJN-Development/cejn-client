<script lang="ts">
	...
	const filterObjects = (e) => {
		filteredObjects = objectArray.filter((object) =>
			object.text.toLowerCase().includes(e.target.value.toLowerCase())
		);
	};

	const isFilteredOut = (key: number, filterResults: objectArrayType[]) => {
		return !filterResults.map((filterResults) => Number(filterResults.key)).includes(Number(key));
	};
...
{#if !expanded}
	...
	>
		...
		<div class="flex-row">
			<Icon name="arrow_down" alt="Arrow on multi select component expander" />
		</div>
	</div>
{:else}
	<div
	...
	>
		...
		/>
		<div class="stack-16">
			<form class="dropdown bordered border-top--square raised" bind:this={formElement}>
				<div
					id="selected-objects"
